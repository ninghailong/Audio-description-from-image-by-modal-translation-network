...
data1 = np.load('/home/user6/project/sound_of_image/data/cifar100/cifar100_sound_representation.npz')
sound_representation = data1['arr_0']

label_data = np.load('/home/user6/project/sound_of_image/data/cifar100/cifar100_train_label.npz')
label = label_data['arr_0']

# Fs = np.transpose(file['Fs'])

# name_list = np.transpose(file['name_list'])
...
with tf.name_scope('Loss'):
    ...
    total_loss = 0.1*generation_loss + 0.1*representation_loss + 0.5*class_loss
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    for epoch in range(training_epochs):
        for batch_idx in range(total_batches):
            data = sample_batch(img_data, sound_data, sound_representation, label, batch_size)
            batch_xs = data[0]
            batch_ys = data[1]
            batch_zs = data[2]
            batch_class_label = data[3]
            # print(batch_xs.shape)
            training_loss, _ = sess.run([total_loss, train],
                                        feed_dict={images_input: batch_xs, labels_input: batch_ys,
                                                   sound_encode_placeholder: batch_zs,
                                                   class_label: batch_class_label,
                                       on_train: True, drop_rate: drop_rate_init})
            training_step += 1
            if training_step % display_step == 0:
                print('Training Epoch:', epoch, ", Training Step:", training_step,
                      ", Training Loss=", training_loss)
        saver.save(sess=sess, save_path='./my_model_cifar100/sound_description.ckpt')
